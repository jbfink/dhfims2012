...
.notes Hey, man, this is a service profession innit? Still here, even with this.So we're planning on having in-person consults, some species of regular office hours, as well as being available online, in some way.
...
!SLIDE bullets incremental
# the stuff we make #
* ... goes up publicly (if possible)
* ... is free software (if possible)
* ... is broadly useful (if possible)

.notes Tell story about going to MIP to talk to IP people. Lucky! Source code as the new peer review. This is an easy way we can demonstrate our utility as well as give back and make a name for ourselves generally. Viz CHNM, with Zotero, etc. *VERY* important that we have license flexibility (DFSG compliant) as we'd like to default to an open source license (as part of SLA? maybe). Github as distribution point.
